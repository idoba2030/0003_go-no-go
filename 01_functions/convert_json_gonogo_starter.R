  ...
  if (sum(grepl(task_name,files))>0) {
    df_starter<-
      data.frame(
        ...
        num_Tries <- fromJSON(file=paste(curnfolder,'/',files[grepl('starter',files)],sep=""))$`data[numTries]`,
        Time <- as.integer(fromJSON(file=paste(curnfolder,'/',files[grepl('starter',files)],sep=""))$`data[endTimeStamp]`)-as.integer(fromJSON(file=paste(curnfolder,'/',files[grepl('starter',files)],sep=""))$`data[startTimeStamp]`)
      )
